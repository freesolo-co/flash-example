...
Structured number guess reaches honest parity with GPT-5.5 on genuine varied-secret binary search. Logic boolean and Sudoku reach near parity under their corrected strict rewards. The old claim that number guess exceeded GPT-5.5 came from trivial constant-offset data and is retracted.
...
Each row uses 50 frozen held-out cases. Train and held-out inputs are disjoint, and held-out prompts were not sent to a teacher.
...
Provider-reported mean total tokens per evaluated case or episode were retained for the original five unaffected evaluations. The corrected Phase 2 evaluation artifacts for logic, number guess, and Sudoku did not retain token usage, so no token number is inferred for those rows.
