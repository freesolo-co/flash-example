...
This example is multi-turn: the model guesses, the environment answers higher or lower, and
the reward reads every guess in the transcript. `flash env eval` drives those turns for a
suite that sets `grades_episodes = True`, and hands the finished rollout state to `score`.
"""
...
class EpisodeEvalSuite:
    ...
    `score(case, response_text)` sees only the last reply, which is one guess out of several. Scoring that would still print a percentage, and the percentage would
