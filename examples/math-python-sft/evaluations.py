...
This example is multi-turn: the model writes python, the environment runs it and returns
the result, and the reward requires BOTH the tool result and the final boxed answer to be
...
class EpisodeEvalSuite:
    ...
    `score(case, response_text)` sees only the last reply, which cannot show whether the
    python tool ever ran. Scoring that would still print a percentage, and the percentage would
