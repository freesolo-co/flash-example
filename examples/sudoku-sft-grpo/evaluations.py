...
This example is multi-turn: each turn places digits, the environment replays the board, and
the reward reads the whole transcript. `flash env eval` drives those turns for a suite that
sets `grades_episodes = True`, and hands the finished rollout state to `score`.
"""
...
class EpisodeEvalSuite:
    ...
    `score(case, response_text)` sees only the last reply, which is one move on a partly
    filled board. Scoring that would still print a percentage, and the percentage would
