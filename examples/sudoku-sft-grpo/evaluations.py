...
`EvalSuite.score(case, response_text)`. Sudoku is solved over up to `max_turns` moves, and its
reward reads the whole transcript, so a single reply cannot produce the number this example
...
_MESSAGE = (
    "sudoku-sft-grpo is a multi-turn environment, and `flash env eval` scores a single "
    "response per case. Run the local multi-turn evaluator instead:\n"
    "  uv run python eval/evaluate_suite.py --example sudoku-sft-grpo "
    "--output eval-results/sudoku-model.json"
)
