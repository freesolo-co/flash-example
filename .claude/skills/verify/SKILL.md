...
3. For `math-python-grpo`, confirm the command refuses execution without `--allow-unsafe-local-code-execution`, permits `--dry-run` without the flag, and prints the host-execution warning when explicitly enabled.
